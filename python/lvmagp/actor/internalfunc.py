...
class GuideImage:
    ...
    def findstars(self):
        ...
        GuideImage.guidestarposition = positions[self.liststarpass]
        if (
            len(self.liststarpass) != self.nstar
        ):  # if there does not exist sufficient stars, just use 1 star.
            GuideImage.guidestarposition = GuideImage.guidestarposition[0, :]

        return GuideImage.guidestarposition  # list ; [[x1, y1], [x2, y2], [x3, y3]]

    def calfwhm(self, findstar=False):
        if findstar:
            self.findstars()

        if len(GuideImage.guidestarposition) == 0:
            pass

        windowradius = 5  # only integer
        starsizelist = []
        for i in range(len(GuideImage.guidestarposition[:, 0])):
            xcenter = int(GuideImage.guidestarposition[i, 0])
            ycenter = int(GuideImage.guidestarposition[i, 1])
